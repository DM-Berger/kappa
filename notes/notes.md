...
pairings of runs.  Given a function $M$ such that
$M(\symbfit{y}_i, \symbfit{y}_j) = m_{ij} \in \mathbb{R}$ for $i > j$, then
we can define a *(pairwise) metric reproducibility distribution*
$\mathcal{M} = \left\{ m_{ij} : i > j \right\}$
